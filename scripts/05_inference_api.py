...
# If ML scores below this, the model strongly believes the file is benign.
# Rules alone cannot override this — avoids FPs from broad API patterns
# (e.g. FileOutputStream, JDBC calls) present in legitimate code.
ML_BENIGN_GATE = 0.15
...
def combined_verdict(ml_score: float, rule: dict, inf_threshold: float) -> tuple[str, str, str]:
    ...
    """
    # ML strongly says benign — don't let broad rules create FPs
    if ml_score is not None and ml_score < ML_BENIGN_GATE:
        return "BENIGN", "BENIGN", "ml_benign"
...
# ---------------------------------------------------------------------------
# JSP rule filter — Jasper adds Servlet/HttpServlet to every compiled JSP;
# strip those hits so they don't push benign JSPs into the fileless path.
# ---------------------------------------------------------------------------

# Points each stripped label contributes (matches _IFACES_SERVLET score above)
_JSP_NOISE_IFACES = {"iface:Servlet", "iface:HttpServlet"}
_JSP_NOISE_SCORE  = 3   # each of these was worth 3 pts in rule_check

def _jsp_rule(rule: dict) -> dict:
    """Return a copy of a rule_check result with Jasper-expected interface hits removed."""
    if not rule["triggered"]:
        return rule
    filtered = [r for r in rule["rules"] if r not in _JSP_NOISE_IFACES]
    removed  = len(rule["rules"]) - len(filtered)
    if removed == 0:
        return rule
    new_score = max(rule["score"] - removed * _JSP_NOISE_SCORE, 0)
    if not filtered:
        return {"triggered": False, "rules": [], "risk": "LOW", "score": 0}
    risk = "HIGH" if new_score >= 6 else "MEDIUM"
    return {"triggered": True, "rules": filtered, "risk": risk, "score": new_score}


# Constant-pool entry lines (e.g. "  #42 = Utf8  ...") — not method headers
_CP_LINE_RE     = re.compile(r'^\s+#\d+\s*=')
# Method header: 2-space indent, has parens, ends with ;
_METHOD_HDR_RE  = re.compile(r'^  \S.*\(.*\).*;\s*$')

def _extract_jspservice_ops(javap_text: str) -> list[str]:
    """Extract opcodes from _jspService only, skipping all Jasper boilerplate methods.
    Falls back to empty list if _jspService is not found (caller uses full ops)."""
    in_service = False
    ops: list[str] = []
    for line in javap_text.splitlines():
        if _CP_LINE_RE.match(line):       # constant pool — skip entirely
            continue
        if _METHOD_HDR_RE.match(line):    # method boundary
            in_service = '_jspService' in line
            continue
        if in_service:
            m = _OPCODE_RE.match(line)
            if m:
...
class Predictor:
    ...
    def predict_from_javap(self, name_path: Path, javap_text: str) -> dict:
        """ML + rule pipeline from pre-computed Jasper javap output.
        Uses all opcodes (matches training: 02e/02f generate images from full Jasper
        bytecode, not just _jspService). Strips Jasper-expected interface hits via
        _jsp_rule so benign JSPs don't get pushed into the fileless detection path."""
        ops: list[str] = []
        for line in javap_text.splitlines():
            m = _OPCODE_RE.match(line)
            if m:
        ...
        rule = _jsp_rule(rule_check(name_path, javap_text))
        ...
        if not ops or len(ops) < 4:
            ...
            return {"verdict": "BENIGN", "tier": "BENIGN", "detection_path": "no_bytecode",
                    "ml_score": None, "rule": rule, "backend": self.backend}
    ...
    def predict_local_jsp(self, class_path: Path) -> dict:
        """ML + rule pipeline for .class compiled by compile_jsp_local.
        Same as predict_from_javap: applies _jsp_rule to strip HttpServlet wrapper
        noise injected by our own servlet template (not Jasper boilerplate)."""
        ops, javap_text = disassemble(class_path)
        if ops is None:
        ...
        rule = _jsp_rule(rule_check(class_path, javap_text))
